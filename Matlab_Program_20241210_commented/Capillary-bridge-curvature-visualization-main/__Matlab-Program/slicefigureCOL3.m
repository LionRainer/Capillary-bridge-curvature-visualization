function[sliceplot] = slicefigureCOL3(Mdata,zbot,ztop,n)

M_in = compressionCOL3(Mdata,n);
...
for i = 1:n
    ...
    data0 = M_in{i};
    data = sortrows(data0,1);
    zval = round(180/pi*mean(data(:,3)),1);
    
    % Plot the GC(a) line for the current z value
    plot(data(:,1),data(:,5),'DisplayName', num2str(zval), 'Color', colors(i, :));
end
...
for i = 1:n
    ...
    % Plot the GC(a) line for the current z value
    plot(data(:,1),data(:,4),'DisplayName', num2str(zval), 'Color', colors(i, :));
end
